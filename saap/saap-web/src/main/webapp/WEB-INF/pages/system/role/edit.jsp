					...
					<div>
						...
						<table class="table table-bordered table-hover datatable">
							...
							<c:forEach items="${permissionTree.subList }" var="domain">
								<tr>
									<td class="checkCol"><input type="checkbox" class="checkItem" id="domain${domain.idCode }" name="permissionCodes" value="${domain.code }" <c:if test="${domain.checked }">checked="checked"</c:if> /></td>						
								...
								<c:forEach items="${domain.subList }" var="level2">
									<c:if test="${level2.type == 2 }">
										<tr>
											<td>&nbsp;</td>						
											<td><input type="checkbox" class="checkItem domain${domain.idCode }" id="module${level2.idCode }" name="permissionCodes" value="${level2.code }" <c:if test="${level2.checked }">checked="checked"</c:if> />&nbsp;<s:message code="module.${level2.code }"></s:message></td>									
										</tr>
										<tr>
											<td>&nbsp;</td>						
											<td>&nbsp;&nbsp;&nbsp;<c:forEach items="${level2.subList }" var="func">											
											<input type="checkbox" class="checkItem  domain${domain.idCode } module${level2.idCode }" name="permissionCodes" value="${func.code }"  <c:if test="${func.checked }">checked="checked"</c:if> />&nbsp;<s:message code="func.${func.code }"></s:message>&nbsp;
											</c:forEach>
											</td>									
										</tr>
									</c:if>
									<c:if test="${level2.type == 1 }">
										<tr>
											<td>&nbsp;</td>						
											<td><input type="checkbox" class="checkItem domain${domain.idCode }" id="domain${level2.idCode }" name="permissionCodes" value="${level2.code }" <c:if test="${level2.checked }">checked="checked"</c:if> />&nbsp;<s:message code="domain.${level2.code }"></s:message></td>									
										</tr>
										<c:forEach items="${level2.subList }" var="level3">
											<tr>
												<td>&nbsp;</td>						
												<td>&nbsp;&nbsp;&nbsp;<input type="checkbox" class="checkItem domain${domain.idCode } domain${level2.idCode }" id="module${level3.idCode }" name="permissionCodes" value="${level3.code }"  <c:if test="${level3.checked }">checked="checked"</c:if> />&nbsp;<s:message code="module.${level3.code }"></s:message></td>									
											</tr>
											<tr>
												<td>&nbsp;</td>							
												<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<c:forEach items="${level3.subList }" var="func">											
												<input type="checkbox" class="checkItem domain${domain.idCode } domain${level2.idCode } module${level3.idCode }" name="permissionCodes" value="${func.code }"  <c:if test="${func.checked }">checked="checked"</c:if> />&nbsp;<s:message code="func.${func.code }"></s:message>&nbsp;
												</c:forEach>
												</td>									
											</tr>
										</c:forEach>										
